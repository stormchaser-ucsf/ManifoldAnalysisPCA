...
%% ANALYSIS 3 -> SHARED VARIANCE BETWEEN ANY TWO MANIFOLDS WITH QR DECOMP
% sticking with the same dummy variables X1, X2, X3 created in prev. cell
...
dim=3;
A = randn(n,dim);
[Qa,~] = qr(A,0); % gets an orthonormal basis for column space of A
% check for orthonormal basis
disp(Qa'*Qa) % the identity matrix
disp(A'*A) % not the identity matrix

% manifold B which is a noisy version of A
scaling=1.5;
B = A + scaling*randn(size(A)); % play around with scaling to see when test fails
[Qb,~] = qr(B,0); % gets an orthonormal basis for column space of B

% shared variance: A relative to B
shared_var=[]; % varied between 0 and 1
num = trace(Qa*Qa' * B*B' * Qa*Qa');
 % think of it this way: if A and B are highly similar, then (Qa*Qa'*B)
 % will basically project B onto its own basis, capturing the same variance
 % as just B originally
den = trace(B*B');
shared_var = [shared_var num/den];

% B relative to A
num = trace(Qb*Qb' * A*A' * Qb*Qb');
den = trace(A*A');
shared_var = [shared_var num/den];

% take the average of the two
shared_var = mean(shared_var);

% null distribution test:
shared_var_boot=[];
for iter=1:5000 % shuffle the entires of A/B, assume A/B to be another random manifold etc.
    shared_var_iter=[];    
    % shuffle A
    A_null = reshape(A(randperm(numel(A))),size(A));
    [Qa,~] = qr(A_null,0);
    [Qb,~] = qr(B,0);
    num = trace(Qa*Qa' * B*B' * Qa*Qa');
    den = trace(B*B');
    shared_var_iter = [shared_var_iter num/den];
    num = trace(Qb*Qb' * A_null*A_null' * Qb*Qb');
    den = trace(A_null*A_null');
    shared_var_iter = [shared_var_iter num/den];

    % shuffle B
    B_null = reshape(B(randperm(numel(B))),size(B));
    [Qb,~] = qr(B_null,0);
    [Qa,~] = qr(A,0);
    num = trace(Qa*Qa' * B_null*B_null' * Qa*Qa');
    den = trace(B_null*B_null');
    shared_var_iter = [shared_var_iter num/den];
    num = trace(Qb*Qb' * A*A' * Qb*Qb');
    den = trace(A*A');
    shared_var_iter = [shared_var_iter num/den];
    ...
    % mean
    shared_var_boot(iter) = mean(shared_var_iter);
end

figure;hist(shared_var_boot)
vline(shared_var)
